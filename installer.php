...
if(isset($_REQUEST['ajax']) && !empty($_REQUEST["ajax"])) {
    ...
    if($_REQUEST['ajax'] == 'status'){
        ...
        $pockethold->phlog('状态： ', $status, 'install.log');
        echo $status;

    }elseif($_REQUEST['ajax'] == 'prepare'){

        $pockethold->prepare();
        $pockethold->phlog('状态： ', 'composer 已被解压', 'install.log');
...
//Removes installer.php
    ...
    <head>
		...
		<title>FlarumChina Composer 安装器</title>
    </head>
    <body>
    <div class="container">
        <div class="jumbotron" style="background-color: transparent;">
            <div class="container text-center">
                ...
                <p style="max-width: 460px; margin:auto;">FlarumChina Composer 安装器(修改来自于Pockethold)是 FlarumChina composer 网页安装器</p>
                <p style="max-width: 460px; margin:auto;">可以不用命令行即可完成composer和flarumchina的安装</p>
        ...
        $(document).ready(function () {
            $.ajax({
                ...
                .fail(function (err) {
                    ...
                    $(".install").replaceWith('<h2 class="instal1">错误:' + err.status + '</h2>');
                });
